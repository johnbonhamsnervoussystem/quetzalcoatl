...
//
  ...
  */
  ...
  do {
    ...
    */
    H_SVD.compute( rho) ;
    std::cout << " rho singular values " << std::endl ;
    std::cout << H_SVD.singularValues() << std::endl ;
    rho = p_rho ;
    rho_i = rho.inverse() ;
    print_mat( rho_i, " rho inverse") ;
    kappa_i = kappa.inverse() ;
    print_mat( kappa_i, " kappa inverse") ;
    ...
    */
    ...
    for ( in = 0; in < inmb; in++){
      ...
      */
      ...
      tmp = h.block( 0, 0, 2*nbas, 2*nbas) + G ;
      f_theta = ( tmp + tmp.adjoint())/z2 ;
